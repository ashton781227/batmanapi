...
class CreatePanic implements ShouldQueue
{
    use Dispatchable, InteractsWithQueue, Queueable, SerializesModels;

    protected $user_id, $data, $system_response, $wayne_variables, $panic_id;
     ...
     * @return void
     */
    public function __construct($user_id, $data, $system_response, $wayne_variables, $panic_id)
    {
    ...
    public function handle()
    {
        ...
        ->accept('application/json')->post('https://wayne.fusebox-staging.co.za/api/v1/panic/create', $this->wayne_variables);

        $external_panic_id = $wayne_response['data'];

        // Add external panic id from wayne Api to DB
        if($wayne_response['data'])
            {
                if ($this->panic_id) {
                    Panic::where('id', $this->panic_id)
                    ->update(['external_panic_id' => $external_panic_id['panic_id']]);
                }
